...
def main(excel_directory,wellname_marker,time_start_marker,time_start_index,time_end_marker,time_end_index,activity_start_marker,activity_end_marker,date_marker,activity_index):
    ...
    for file_name in tqdm(os.listdir(excel_directory)):
        ...
        if file_name.endswith('.xlsx'):
            ...
            for sheet_name, sheet_df in all_sheets.items():
                ...
                well_marker_index = np.where(sheet_df == wellname_marker)
                if len(well_marker_index[0]) > 0 and len(well_marker_index[1]) > 0:
                    row_index = well_marker_index[0][0]
                    col_index = well_marker_index[1][0]

                    # Find the cell one column to the right of the "Well:" marker
                    next_cell_value = sheet_df.iloc[row_index, col_index + 1]

                    # Continue searching to the right until a non-empty cell is found
                    while pd.isnull(next_cell_value) or next_cell_value == "":
                        col_index += 1
                        next_cell_value = sheet_df.iloc[row_index, col_index]

                    # Now, row_index and col_index contain the indices of the first non-empty cell to the right
                    result_row_index = row_index
                    result_col_index = col_index
                    result_value = next_cell_value
                else:
                ...
                if from_row is not None or to_row is not None or operation_row is not None or next_projection_row is not None:
                    ...
                    new_df = pd.DataFrame({
                        ...
                        'WELL' : sheet_df.iloc[result_row_index, result_col_index],
                    ...
                    try:
                        ...
                        if len(date_cell) > 0:
                            ...
                            date_match = re.search(r'\b(?:\d{1,2}[./-]\d{1,2}[./-]\d{2,4}|\w+\s\d{1,2},?\s\d{2,4}|\d{4}-\d{2}-\d{2}|\d{2}\.\d{2}\.\d{4}|\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}|\d{1,2}:\d{2}:\d{2}\s[APMapm]{2})\b',
                    ...
                    if not new_df['OPERATION'].isnull().all():
                    ...
                    new_df[['START', 'END']] = new_df[['START', 'END']].fillna('')
                    ...
                    try:
                        ...
                        print("Error: Unable to convert 'DATE' column to datetime objects.")
                    ...
                    if date_match is not None:
                        ...
                        # If 'DATE' column is empty, append empty strings to 'START_DATE' and 'END_DATE'
                        new_df['START_DATE'] = new_df['START']
                        new_df['END_DATE'] = new_df['END']
    ...
    final_df.insert(0, 'WellGUID', '')
    # final_df.insert(1, 'WellName', wname)
    final_df.insert(2, 'WellboreName', '')
    ...
    column_mapping = {'OPERATION': 'Description', 'START_DATE': 'StartDate', 'END_DATE': 'EndDate', 'START_DEPTH': 'TopDepthMD', 'END_DEPTH': 'BottomDepthMD',
                       'HOLE_SIZE': 'WellboreDiameter', 'Class_Type': 'TimeClassification','NPT CODE':'NPTDetail','ACTIVITY CODE':'ActivityCodeL1','WELL':'WellName'}
...
if __name__ == "__main__":
    ...
    # wguid = sys.argv[2]
    wellname_marker = sys.argv[2]
    # wbname = sys.argv[4]
    time_start_marker = sys.argv[3]
    time_start_index = sys.argv[4]
    time_end_marker = sys.argv[5]
    time_end_index = sys.argv[6]
    activity_start_marker = sys.argv[7]
    activity_end_marker = sys.argv[8]
    date_marker = sys.argv[9]
    activity_index = sys.argv[10]
    ...
    main(excel_directory, wellname_marker, time_start_marker,time_start_index,time_end_marker,time_end_index,activity_start_marker,
