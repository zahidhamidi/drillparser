...
def main():
    ...
    if uploaded_files: 
        ...
        with col33:
            ...
            st.subheader("Template Selection",divider=True)
            st.write("Select your expected .xlsx output template")

            output_template = st.radio("Select Extraction Template", ["No Selection","DrillPlan","Performance Insight"])
            company_template = st.selectbox("Select Custom Client Template", ["No Selection","PETRONAS","Saudi Aramco","MPCL"])

        with col44:

            st.subheader("Coordinate Check (Optional)",divider=True)
        ...
        def alphabet_to_index(alphabet):
            ...
            if alphabet != '':
                # Convert alphabet to uppercase to make it case-insensitive
                uppercase_alphabet = alphabet.upper()

                # Calculate the index based on ASCII value
                index = ord(uppercase_alphabet) - ord('A')
        ...
        with col2:
            ...
            time_end_index = st.text_input("TimeEnd Index",value='')
            time_end_index = alphabet_to_index(time_end_index)
        ...
        with col3:
            ...
            activity_index = st.text_input("Activity Index",value='')
            activity_index = alphabet_to_index(activity_index)
        ...
        if st.button("Start Parse"):
            ...
            if (output_template == "DrillPlan") & (company_template == "No Selection"):
                subprocess.run(["python", "Home.py", temp_directory, wellname_marker, time_start_marker,time_start_index,time_end_marker,time_end_index,
